...
BEGIN
 ...
 CASE _operation 
    ...
    WHEN 5 THEN 
    IF EXISTS (SELECT * FROM table_tutor_assignment WHERE tutor_assignment_id=_tutor_assignment_id AND deleted IS NOT NULL) THEN
...
DELIMITER $$
DROP PROCEDURE `pgaeng`.`list_table_tutor_assignment`;
CREATE PROCEDURE list_table_tutor_assignment(
