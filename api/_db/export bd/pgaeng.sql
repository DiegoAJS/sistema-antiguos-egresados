...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 15-05-2019 a las 15:32:26
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `crud_table_person` (IN `_operation` INT, IN `_person_id` INT, IN `_first_name` VARCHAR(255), IN `_last_name` VARCHAR(255), IN `_ci` VARCHAR(120), IN `_email` VARCHAR(120), IN `_cellphone` VARCHAR(16), IN `_telephone` VARCHAR(16), IN `_city` VARCHAR(120), IN `_address` VARCHAR(120), IN `_passwords` VARCHAR(120))  BEGIN
 ...
 CASE _operation
    ...
    /* A C T  U A L I Z A R */
	WHEN 3 THEN
		IF EXISTS(SELECT * FROM table_person WHERE table_person.person_id=_person_id) THEN
			UPDATE table_person
    ...
    /* LO G I N */
    WHEN 8 THEN
		IF EXISTS(select table_person.id from table_person WHERE  ci = _ci AND passwords = _passwords) THEN
    ...
    /* C A R  G O S */
    WHEN 9 THEN
		IF EXISTS(select b.appointment from table_person a INNER JOIN table_appointment b on a.person_id = b.person_id WHERE b.person_id=_person_id) THEN
	...
	WHEN 10 THEN
		IF EXISTS(select a.* from table_person a INNER JOIN table_tutor_assignment b on a.person_id = b.person_tutor_id WHERE b.person_tutor_id=_person_id) THEN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `crud_table_type` (IN `_operation` INT, IN `_type_id` INT, IN `_name_type` VARCHAR(255), IN `_label_type` VARCHAR(255), IN `_foreign_type_id` INT)  BEGIN
 ...
 CASE _operation 
	WHEN 1 THEN 
		INSERT INTO table_type(name_type, label_type, foreign_type_id)
		SELECT * FROM (SELECT  _name_type, _label_type, _foreign_type_id) AS tmp
		WHERE NOT EXISTS (SELECT * FROM table_type WHERE name_type= _name_type
														AND label_type=_label_type) LIMIT 1; 
        IF EXISTS(SELECT type_id FROM table_type WHERE name_type= _name_type
														AND label_type=_label_type) THEN
			SELECT type_id FROM table_type WHERE name_type= _name_type
														AND label_type=_label_type;
		END IF;
	WHEN 2 THEN
		SELECT * FROM table_type WHERE table_type.type_id=_type_id LIMIT 1;
	...
	WHEN 3 THEN 
        ...
        SELECT type_id FROM table_type WHERE table_type.type_id = _type_id;  
        
	WHEN 4 THEN 
		DELETE FROM table_type WHERE table_type.type_id = _type_id LIMIT 1;
        SELECT "Eliminado exitoso" AS msg;
