...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 05-06-2019 a las 16:26:50
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `crud_table_advance` (IN `_operation` INT, IN `_control_id` INT, IN `_student_record_id` INT, IN `_type_id` INT, IN `_value_control` BOOLEAN)  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `crud_table_payment_history` (IN `_operation` INT, IN `_payment_history_id` INT, IN `_student_record_id` INT, IN `_payment` DECIMAL, IN `_payment_code` VARCHAR(16), IN `_name_bank` VARCHAR(26))  BEGIN
 ...
 CASE _operation 
	...
	ELSE
		SELECT payment_history_id, FALSE AS "status", "Error ya existe registro" AS msg;
	END IF;
	WHEN 2 THEN
	...
	ELSE
		SELECT payment_history_id, FALSE AS "status", "Error no existe registro" AS msg;
	END IF;
    ...
    UPDATE table_payment_history
        ...
        SELECT payment_history_id, TRUE AS "status", "Correcto registro EDITADO" FROM table_payment_history WHERE payment_history_id=_payment_history_id LIMIT 1;  
	ELSE
		SELECT payment_history_id, FALSE AS "status", "Error no existe registro" AS msg;
    END IF;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `crud_table_position` (IN `_operation` INT, IN `_position_id` INT, IN `_person_id` INT, IN `_appointment` VARCHAR(120), IN `_profession` TEXT, IN `_specialty` TEXT, IN `_career_direction` VARCHAR(120))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `crud_table_student_record` (IN `_operation` INT, IN `_student_record_id` INT, IN `_person_student_id` INT, IN `_program_version_id` INT, IN `_career` VARCHAR(255), IN `_career_direction` VARCHAR(511), IN `_cu` VARCHAR(15), IN `_cost` DECIMAL, IN `_type_register` VARCHAR(120), IN `_state` VARCHAR(120))  BEGIN
 ...
 CASE _operation 
	...
	IF NOT EXISTS (SELECT * FROM table_student_record WHERE person_student_id = _person_student_id  
        ...
        SELECT student_record_id, TRUE AS "status", "Correcto registro CREADO" AS msg FROM table_student_record WHERE person_student_id = _person_student_id AND program_version_id=_program_version_id AND career=_career LIMIT 1;
	ELSE
    ...
    IF EXISTS (SELECT * FROM table_student_record WHERE student_record_id=_student_record_id AND deleted IS NULL) THEN 
		...
		SELECT student_record_id, TRUE AS "status", "Correcto registro EDITADO" AS msg  FROM table_student_record WHERE student_record_id=_student_record_id LIMIT 1;
	ELSE 
    ...
    WHEN 8 THEN
		IF EXISTS(select a.* from table_person a INNER JOIN table_student_record b on a.person_id = b.person_student_id WHERE career_direction=_career_direction AND b.deleted IS NULL) THEN
			select DISTINCT a.* from table_person a INNER JOIN table_student_record b on a.person_id = b.person_student_id WHERE career_direction=_career_direction;
		END IF;	
    
    /* ESTUDIANTES CARRERAS*/
	WHEN 9 THEN 
    IF EXISTS(select DISTINCT b.career from table_person a INNER JOIN table_student_record b on a.person_id = b.person_student_id WHERE a.person_id=_person_student_id AND b.deleted IS NULL) THEN
		select DISTINCT b.career, b.cu from table_person a INNER JOIN table_student_record b on a.person_id = b.person_student_id WHERE a.person_id=_person_student_id AND b.deleted IS NULL;
    END IF;
     
     /* Estudiante carrera  version */
     WHEN 10 THEN 
     IF EXISTS(SELECT b.* FROM table_student_record a INNER JOIN table_program_version b ON a.program_version_id = b.program_version_id WHERE a.person_student_id=_person_student_id AND a.career=_career AND a.deleted IS NULL) THEN 
		SELECT a.person_student_id, b.* FROM table_student_record a INNER JOIN table_program_version b ON a.program_version_id = b.program_version_id WHERE a.person_student_id=_person_student_id AND a.career=_career AND a.deleted IS NULL;
     END IF;
...
--
-- Estructura de tabla para la tabla `table_advance`
--

CREATE TABLE `table_advance` (
  `advance_id` int(11) NOT NULL,
  `student_record_id` int(11) NOT NULL,
  `modality` varchar(120) DEFAULT NULL,
...
(1, 'Jamil', 'Bogan', '319275097', 'santiago.dietrich@example.net', '399.380.5699', '1-117-477-8224', 'Jonesview', '8167 Hillary Court\nNorth Kirstinmouth, VT 55643-5736', '6436a7dca26cdcc68d2632dc4932e331cf563aa3', '2006-04-02 03:45:05', '2011-12-24 00:29:26', NULL),
...
--
-- Estructura de tabla para la tabla `table_position`
--
...
CREATE TABLE `table_position` (
  ...
  `career_direction` varchar(511) DEFAULT NULL,
  `created` datetime DEFAULT CURRENT_TIMESTAMP,
  `updated` datetime DEFAULT CURRENT_TIMESTAMP,
  `deleted` datetime DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
--
-- Volcado de datos para la tabla `table_position`
--

INSERT INTO `table_position` (`position_id`, `person_id`, `appointment`, `profession`, `specialty`, `career_direction`, `created`, `updated`, `deleted`) VALUES
(1, 1, 'ROOT', 'Licenciatura en Ingeniería de Sistemas', 'Desarrollo de Software', 'Dirección  de las carreras de ing. De sistemas, animación digital, tecnologías de la información y ciencias de la computación', '2019-06-02 09:25:21', '2019-06-02 09:25:21', NULL),
(2, 2, 'Coordinador', 'Licenciatura en Ingeniería En Ciencias En Computación', 'Network, Inteligencia Artificial', 'Dirección de carrera de ingeniería en telecomunicaciones', '2019-06-02 09:25:21', '2019-06-02 09:25:21', NULL),
(3, 3, 'Coordinador', 'Licenciatura en Ingeniería Química', NULL, 'Dirección  de la carrera de ingeniería química', '2019-06-02 09:25:21', '2019-06-02 09:25:21', NULL),
(4, 4, 'Coordinador', 'Licenciatura en Ingeniería Industrial', NULL, 'Dirección  de carrera ingeniería industrial', '2019-06-02 09:25:21', '2019-06-02 09:25:21', NULL),
(5, 5, 'Coordinador', 'Licenciatura en Ingeniería Ambiental ', 'Desarrollo de Software', 'Dirección  de carrera de ingeniería ambiental', '2019-06-02 09:25:21', '2019-06-02 09:25:21', NULL),
(6, 6, 'Coordinador', 'Licenciatura en Ingeniería Mecánica', NULL, 'Dirección  de la carrera de ingeniería mecánica', '2019-06-02 09:25:21', '2019-06-02 09:25:21', NULL),
(7, 7, 'Coordinador', 'Licenciatura en Ingeniería Eléctronica', NULL, 'Dirección  de las carreras de ingeniería electrónica e ingeniería mecatrónica', '2019-06-02 09:25:21', '2019-06-02 09:25:21', NULL),
(8, 8, 'Coordinador', 'Licenciatura en Ingeniería Electromecánica', NULL, 'Dirección de carrera de ingeniería electromecánica', '2019-06-02 09:25:21', '2019-06-02 09:25:21', NULL),
(9, 9, 'Coordinador', 'Técnico Superior en Informática', NULL, 'Dirección de carrera de informática T.S.', '2019-06-02 09:25:21', '2019-06-02 09:25:21', NULL),
(10, 10, 'Coordinador', 'Licenciatura en Ingeniería Química', NULL, 'Dirección de carrera de química industrial T.S', '2019-06-02 09:25:21', '2019-06-02 09:25:21', NULL),
(11, 11, 'Coordinador', 'Licenciatura en Ingeniería de Telecomunicaciones', NULL, 'Dirección de carrera de ingeniería en telecomunicaciones', '2019-06-02 09:25:21', '2019-06-02 09:25:21', NULL),
(12, 12, 'Tutor', 'Licenciatura en Ingeniería de Sistemas', 'Robotica', 'Dirección  de las carreras de ing. De sistemas, animación digital, tecnologías de la información y ciencias de la computación', '2019-06-02 09:25:21', '2019-06-02 09:25:21', NULL),
(13, 13, 'Tutor', 'Licenciatura en Ingeniería de Sistemas', 'Desarrollo de Software', 'Dirección  de las carreras de ing. De sistemas, animación digital, tecnologías de la información y ciencias de la computación', '2019-06-02 09:25:21', '2019-06-02 09:25:21', NULL),
(14, 14, 'Tutor', 'Licenciatura en Ingeniería de Sistemas', 'Robotica', 'Dirección  de las carreras de ing. De sistemas, animación digital, tecnologías de la información y ciencias de la computación', '2019-06-02 09:25:21', '2019-06-02 09:25:21', NULL),
(15, 15, 'Tutor', 'Licenciatura en Ingeniería de Sistemas', 'Desarrollo de Software', 'Dirección  de las carreras de ing. De sistemas, animación digital, tecnologías de la información y ciencias de la computación', '2019-06-02 09:25:21', '2019-06-02 09:25:21', NULL),
(16, 16, 'Tutor', 'Licenciatura en Ingeniería de Sistemas', 'Seguridad de la informacion', 'Dirección  de las carreras de ing. De sistemas, animación digital, tecnologías de la información y ciencias de la computación', '2019-06-02 09:25:21', '2019-06-02 09:25:21', NULL),
(17, 17, 'Tutor', 'Licenciatura en Ingeniería de Sistemas', 'Desarrollo de Software', 'Dirección  de las carreras de ing. De sistemas, animación digital, tecnologías de la información y ciencias de la computación', '2019-06-02 09:25:21', '2019-06-02 09:25:21', NULL),
(18, 18, 'Tutor', 'Licenciatura en Ingeniería de Sistemas', 'Desarrollo de Software', 'Dirección  de las carreras de ing. De sistemas, animación digital, tecnologías de la información y ciencias de la computación', '2019-06-02 09:25:21', '2019-06-02 09:25:21', NULL),
(19, 19, 'Tutor', 'Licenciatura en Ingeniería de Sistemas', 'Network, Inteligencia Artificial', 'Dirección  de las carreras de ing. De sistemas, animación digital, tecnologías de la información y ciencias de la computación', '2019-06-02 09:25:21', '2019-06-02 09:25:21', NULL),
(20, 20, 'Tutor', 'Licenciatura en Ingeniería de Sistemas', 'Desarrollo de Software', 'Dirección  de las carreras de ing. De sistemas, animación digital, tecnologías de la información y ciencias de la computación', '2019-06-02 09:25:21', '2019-06-02 09:25:21', NULL),
(21, 21, 'Tutor', 'Licenciatura en Ingeniería de Sistemas', 'Sistemas Integrados', 'Dirección  de las carreras de ing. De sistemas, animación digital, tecnologías de la información y ciencias de la computación', '2019-06-02 09:25:21', '2019-06-02 09:25:21', NULL),
(22, 22, 'Tutor', 'Licenciatura en Ingeniería de Sistemas', 'Dispositivos moviles', 'Dirección  de las carreras de ing. De sistemas, animación digital, tecnologías de la información y ciencias de la computación', '2019-06-02 09:25:21', '2019-06-02 09:25:21', NULL),
(23, 23, 'Tutor', 'Licenciatura en Ingeniería de Sistemas', 'Desarrollo de Software', 'Dirección  de las carreras de ing. De sistemas, animación digital, tecnologías de la información y ciencias de la computación', '2019-06-02 09:25:21', '2019-06-02 09:25:21', NULL),
(24, 24, 'Tutor', 'Licenciatura en Ingeniería de Sistemas', 'UX y diseño', 'Dirección  de las carreras de ing. De sistemas, animación digital, tecnologías de la información y ciencias de la computación', '2019-06-02 09:25:21', '2019-06-02 09:25:21', NULL),
(25, 25, 'Tutor', 'Licenciatura en Ingeniería de Sistemas', NULL, 'Dirección  de las carreras de ing. De sistemas, animación digital, tecnologías de la información y ciencias de la computación', '2019-06-02 09:25:21', '2019-06-02 09:25:21', NULL),
(26, 26, 'Tutor', 'Licenciatura en Ingeniería de Sistemas', NULL, 'Dirección  de las carreras de ing. De sistemas, animación digital, tecnologías de la información y ciencias de la computación', '2019-06-02 09:25:21', '2019-06-02 09:25:21', NULL);
...
--
-- Volcado de datos para la tabla `table_student_record`
--

INSERT INTO `table_student_record` (`student_record_id`, `person_student_id`, `program_version_id`, `career`, `career_direction`, `cu`, `cost`, `on_account`, `type_register`, `state`, `created`, `updated`, `deleted`) VALUES
(1, 27, 1, 'Sistemas', 'Direccion de sistemas', '35-1234', '100000', '0', 'nuevo', 'habilitado', '2019-06-02 09:49:06', '2019-06-02 09:49:06', NULL),
(2, 27, 3, 'Sistemas', 'Direccion de sistemas', '35-1234', '100000', '0', 'nuevo', 'habilitado', '2019-06-02 09:49:32', '2019-06-02 09:49:32', NULL),
(3, 27, 4, 'Sistemas', 'Direccion de sistemas', '35-1234', '100000', '0', 'nuevo', 'habilitado', '2019-06-02 09:50:03', '2019-06-02 09:50:03', NULL),
(4, 27, 4, 'Computacion', 'Direccion de sistemas', '36-1234', '100000', '0', 'nuevo', 'habilitado', '2019-06-02 14:27:03', '2019-06-02 14:27:03', NULL);
...
CREATE TABLE `table_tracing` (
  ...
  `documents_attached` varchar(255) DEFAULT NULL,
  `created` datetime DEFAULT CURRENT_TIMESTAMP,
  `updated` datetime DEFAULT CURRENT_TIMESTAMP,
  `deleted` datetime DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
--
-- Indices de la tabla `table_advance`
--
ALTER TABLE `table_advance`
...
--
-- Indices de la tabla `table_position`
--
...
--
-- AUTO_INCREMENT de la tabla `table_advance`
--
ALTER TABLE `table_advance`
...
--
-- AUTO_INCREMENT de la tabla `table_position`
--
ALTER TABLE `table_position`
...
--
ALTER TABLE `table_student_record`
  MODIFY `student_record_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5;
...
--
-- Filtros para la tabla `table_advance`
--
ALTER TABLE `table_advance`
  ADD CONSTRAINT `table_advance_ibfk_1` FOREIGN KEY (`student_record_id`) REFERENCES `table_student_record` (`student_record_id`);
...
--
-- Filtros para la tabla `table_position`
--
ALTER TABLE `table_position`
  ADD CONSTRAINT `table_position_ibfk_1` FOREIGN KEY (`person_id`) REFERENCES `table_person` (`person_id`);
